...
//import com.fc.simulateAnnelingSeedAndConstraints.AnnelProcessSeedConstraint;
...
//		List<Tuple> mfs = new ArrayList<Tuple> ();
		int[][] seed = new int[][]{};
//		Process aetg_all = new Process(seed, mfs);
//		Process t = new Process(seed, mfs );
...
//				System.out.println(testCase.getStringOfTest());
			...
			HashSet<Integer> differentStates = new HashSet<Integer> ();
			int cur = 0;
			for (TestCase testCase : overallTestCases) {
				...
				state[cur] = "" + runresult;
				differentStates.add(runresult);
				cur++;
			}

			CTA cta = new CTA();
			String[] classes = new String[differentStates.size()];
			// classes[0] = "0";
			int j = 0;
			for (Integer r : differentStates) {
				classes[j] = r + "";
				j++;
			}
...
//			}
			...
			List<int[]> tempR = new ArrayList<int[]> ();
			for (int[] test : aetg.rsTable) {
				TestCase testCase = new TestCaseImplement(test);
				if (caseRunner.runTestCase(testCase) == 0) {
					tempR.add(test);
