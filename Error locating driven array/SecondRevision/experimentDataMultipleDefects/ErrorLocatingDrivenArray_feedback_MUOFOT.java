...
//			
			...
			if (failureCode == 0) {
				...
				List<Tuple> mfs = getMFS(ac, testCase, failureCode);
...
//					}
	...
	public List<List<Tuple>> checkMFS(List<Tuple> mfs, AETG_Constraints ac,
			TestCase testCase, HashMap<Tuple, HashSet<TestCase>>  executed, CaseRunner caseRunner) {
		...
		for (Tuple tuple : mfs) {
			...
			if (isMFSWrong(ac, tuple, testCase, executed.get(revser), caseRunner)) {
	...
	public List<Tuple> getMFS(AETG_Constraints ac, TestCase testCase, int failureCode) {
		...
		List<Tuple> temp_mfs = reLocate(ac, sc, new ArrayList<Tuple>(), caseRunner);
		...
		List<List<Tuple>> checkedNonMFS = this.checkMFS(temp_mfs, ac, testCase,
				sc.getTestedTupleTestCases(), caseRunner);
		...
		while (right.size() == 0) { // right is large than 0 , or wrong is 0,
			...
			temp_mfs = reLocate(ac, sc, right, caseRunner);
	...
	public List<Tuple> reLocate(AETG_Constraints ac, FIC_Constraints sc,
			List<Tuple> addedMFS, CaseRunner caseRunner) {
		sc.addMFS(addedMFS);
		sc.FicSingleMuOFOT();
...
//		System.out.println("size" + );
	...
	boolean isMFSWrong(AETG_Constraints ac, Tuple MFS, TestCase wrongCase,
			HashSet<TestCase> executed, CaseRunner caseRunner) {
		...
		if (caseRunner.runTestCase(newCase) == TestCase.PASSED) {
			...
			if (caseRunner.runTestCase(newCase2) == TestCase.PASSED) {
				ac.unCovered = cm.setCover(ac.unCovered, ac.coveredMark,
