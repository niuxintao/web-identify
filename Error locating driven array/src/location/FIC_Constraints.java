...
public class FIC_Constraints {
	...
	private List<Integer> CFree = new ArrayList<Integer>();
	private Tuple partBug;

	private int low;
	private int high;
	private int middle;
	private boolean start = true;
	private boolean startNotProper = false;
	private List<Integer> candList;
	private Tuple freeTuple;
	private Tuple Temp;
	private Pa pa;
	private List<Integer> CTabu = new ArrayList<Integer>();
	private boolean trigger = true;
	private boolean triggerNotProper = false;
	private int[] Ccand;
	...
	public void setLastTestCase(boolean state) {
		// System.out.println("trigger " + trigger + " start " + start);
		if (lastCase != null) {
			lastCase.setTestState(state ? TestCase.PASSED : TestCase.FAILED);
			if (!this.tested.containsKey(lastCase)) {
				this.tested.put(lastCase,
						testCase.testDescription() == TestCase.PASSED);
				extraCases.addTest(lastCase);
			}
		}
		if (trigger) {
			// System.out.println("ha");
			if (state)
				triggerNotProper = true;
			else
				trigger = false;
			...
			if (state) {
				...
				// partBug = new Tuple(0, testCase);
				start = false; // can go on
				low = 0;
				high = Ccand.length - 1;
				middle = (low + high) / 2;
				candList = this.CovertTntToTnteger(Ccand);
			} else {
				// no one
	...
	public TestCase generateNext() {
		lastCase = this.generateNextTestCase();
		if (this.tested.containsKey(lastCase)) {
			this.setLastTestCase(this.tested.get(lastCase));
	...
	public TestCase generateNextTestCase() {
		if (trigger) {
			if (triggerNotProper) {
				triggerNotProper = false;
				return null;
			}
			if (CTabu.size() > 0) {
				return Motivate(CTabu);
			}
			partBug = new Tuple(0, testCase);
			trigger = false;
		}
		if (start) {
			...
			if (Ccand == null || Ccand.length == 0) {
				...
				if (partBug.getDegree() > 0)
					return Motivate(CovertTntToTnteger(determine
							.getParamIndex()));
				else {
					setLastTestCase(true);
					return this.generateNextTestCase();
			...
			if (low < high) {
				...
				Temp = freeTuple.catComm(freeTuple, Low);
				return Motivate(this.CovertTntToTnteger(Temp.getParamIndex()));
			} else {
	...
	public TestCase changeFactor() {
		start = true;
		CFree = pa.CFree;
		Tuple newRelatedPartBug = pa.fixdOne;
		// System.out.println("part"+newRelatedPartBug.toString());
		if (newRelatedPartBug.getDegree() == 0) {
			...
			CFree.addAll(CTabu);
			trigger = true;
			return generateNextTestCase();
		}
		partBug = partBug.catComm(partBug, newRelatedPartBug);
		// if(partBug.getDegree() == 0)
		// return null;
		// else
		// System.out.println(partBug.toString());
		return generateNextTestCase();
	}
	...
	public FIC_Constraints(TestCase testCase, int[] param, CaseRunner caseRunner) {
	...
	protected boolean runTest(TestCase testCase) {
		// System.out.println("str:" + testCase.getStringOfTest());
	...
	protected boolean runTest2(TestCase testCase) {
		// System.out.println("str:" + testCase.getStringOfTest());
		if (this.tested.containsKey(testCase))
			return this.tested.get(testCase);
		else {
...
//					testCase.testDescription() == TestCase.PASSED);
	...
	public static void main(String[] args) {
		...
		System.out.println(fic.getExtraCases().getTestCaseNum());
		...
		FIC_Constraints fic2 = new FIC_Constraints(wrongCase, param, caseRunner);
		TestCase testCase2 = fic2.generateNext();
		while (testCase2 != null) {
			fic2.setLastTestCase(fic2.runTest2(testCase2));
			testCase2 = fic2.generateNext();
		}

		System.out.println(fic2.getExtraCases().getTestCaseNum());

		for (int i = 0; i < fic2.getExtraCases().getTestCaseNum(); i++) {
			System.out.println(fic2.getExtraCases().getAt(i).getStringOfTest());
		}
		for (Tuple bug : fic2.getBugs()) {
			// if (bug.equals(bugModel1) || bug.equals(bugModel2)) {
			// System.out.println("true");
			// }
			System.out.println(bug.toString());
