...
public class TestScrollForAllConfigs {
	...
	public TestScrollForAllConfigs() {
	...
	public String testStatWithResultSet(int statMode, int havePlaceOrNot,
		...
		try {
			TestScroll tsl = new TestScroll(m_conn);
		...
		for (int i = 0; i < set[index]; i++) {
			...
			if (nextIndex == set.length) {
				...
				try {
					...
					if (binValue[0] == 0) {
						throw new Exception(
								"Cannot scoll the resultset which types is  TYPE_FORWARD_ONLY");
					}
...
//		int[] set = { 1, 1, 2, 1, 1 };
	...
	public static void main(String[] args) throws Exception {
		TestScrollForAllConfigs tb = new TestScrollForAllConfigs();
		tb.test();
	}
